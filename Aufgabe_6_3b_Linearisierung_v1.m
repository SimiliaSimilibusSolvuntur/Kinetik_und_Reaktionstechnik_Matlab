...
p = polyfit(t_cut, lin_A_1, 1);

% Geschwindigkeitskonstante 1. Ordnung:
k_1 = -p(1) % [1/s]

% Bestimmtheitsmass für die lineare Regression
R_square = corrcoef(t_cut, lin_A_1).^2;
R_square(1, 2);

% Plot figure 1
figure; hold on; grid on; grid minor;
plot(t_cut, lin_A_1, ':o', 'linewidth', 1);
hold on
...
% Lineare Kurvenanpassung -> Gerade y = p(1)*x + p(2)
p = polyfit(t_cut, lin_A_2, 1);

% Geschwindigkeitskonstante 2. Ordnung:
k_2 = p(1) % [L/(mol*s)]

% Bestimmtheitsmass für die lineare Regression
R_square = corrcoef(t_cut, lin_A_2).^2;
R_square(1, 2);

% Plot figure 2
figure; hold on; grid on; grid minor;
plot(t_cut, lin_A_2, ':o', 'linewidth', 1);
hold on
plot(t_cut, polyval(p, t_cut), '-', 'linewidth', 1);
xlabel('Zeit t [s]');
...
text(200, 200, ['R^2 = ', num2str(R_square(1, 2)), newline, ...
...
text(200, -0.8, ['R^2 = ', num2str(R_square(1, 2)), newline, ...
