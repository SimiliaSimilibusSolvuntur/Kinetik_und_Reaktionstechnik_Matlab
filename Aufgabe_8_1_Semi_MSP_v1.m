...
V_RM_0 = 0.85/1;         % [l] Reaktionsvolumen 
m_RM_0 = 0.85;     % [kg] Reaktionsmasse 
n_A_0 = c_A_0 * V_RM_0;       % [mol] Stoffmenge Komponente B 
% n_Lsm_0 = (m_RM_0 * 1000 - n_A_0 * mw_i(1)) / mw_i(5) % [mol] Stoffmenge Lsm
n_Lsm_0 = m_i_0(5)/mw_i(5);
n_i_0 = [n_A_0, 0, 0, 0, n_Lsm_0];  % [mol] Stoffmenge Komponente i

%% Reaktionsparameter zur Beschreibung der Dosierung
t_dos_start = 0; % [s] Startzeit der Dosierung
t_dos = 3600;   % [s] Dauer der Dosierung
n_dos_B = 1.5; % [mol] Stoffmenge Komponente B dosiert
...
t_end = 8000; % [s] Stop-Zeit der Simulation
...
figure
plot(Sim.t, Sim.m_RM_t)
ylabel('m_{RM} [kg]');
grid on; grid minor
xlabel('Zeit [s]');
title(['Aufgabe 8.1: Reaktion bei 60 °C im Semibatch']);
